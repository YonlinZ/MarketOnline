...
namespace MarketOnline.DB
{
    public class DBHelper
    {
        ...
        {
            var klineTableName = $"C_{symbol}_{interval}_Kline";
            var klineRawTableName = $"C_{symbol}_{interval}_Kline_Raw";
            ...
            {
                var sql2 = GetSQL.GetTableSql(symbol, interval, "Kline", klineTableName);
            ...
            {
                var sql2 = GetSQL.GetTableSql(symbol, interval, "Kline_Raw", klineRawTableName);
                DapperUtil.Execute(DataBaseType.SQLITE, ConstVar.Conn, sql2);
            }
                ...
                try
                {
                    ...
                    foreach (var k in kline)
                    {
                        ...
                        sql = $@"insert into {klineRawTableName}(
                                    ...
                                    TakerBuyQuoteAssetVolume,
                                    Ignore
                                    ) values(datetime('{k[0].ToString().StampToDatetime(true).ToString("yyyy-MM-dd HH:mm:ss.fff")}'),
                                    '{k[1]}','{k[2]}','{k[3]}','{k[4]}','{k[5]}',
